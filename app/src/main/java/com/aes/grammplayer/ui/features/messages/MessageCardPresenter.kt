...
class MessageCardPresenter : Presenter() {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, item: Any) {
        ...
        val spannable = SpannableStringBuilder().apply {
            if(localFileExistsAndIsPlayable){
                val titleBannerStart = this.length
                if(item.isDownloadActive){
                    append("Downloading")
                }else{
                    append("Available locally")
                }
                val titleBannerEnd = this.length
                setSpan(StyleSpan(Typeface.BOLD), titleBannerStart, titleBannerEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            ...
            val titleStart = this.length
            append(title)
            val titleEnd = this.length
            ...
            val fileIdStart = this.length
            append(fileId)
            val fileIdEnd = this.length
            setSpan(StyleSpan(Typeface.BOLD), fileIdStart, fileIdEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            ...
            val sizeStart = this.length
            append(size)
            val sizeEnd = this.length
            setSpan(StyleSpan(Typeface.BOLD), sizeStart, sizeEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
     ...
     */
    class CardViewHolder(view: View, val textView: TextView) : ViewHolder(view)
